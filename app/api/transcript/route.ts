...
export async function GET(request: NextRequest) {
  try {
    ...
    const { messages, rawLines, rawEntries, planSlug, planInsertAfter, numCompactions, pendingAskUserQuestion, currentModel, totalOutputTokens, usageEvents } = parseTranscriptJsonl(content);
    ...
    archiveTranscript(sanitizedSessionId, project, display.substring(0, 200), content, messages, rawLines, undefined, { numCompactions, totalOutputTokens, usageEvents })
